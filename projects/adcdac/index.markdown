...
converges on a digital output. Starting with the MSB, the ADC uses a sample and hold comparator to compare the analog input voltage $V_{IN}$ with an internally generated reference voltage $V_{ref,n}\ =\ (1/2)V_{REF}$. If the analog input voltage $V_{IN}$ is greater than the internally generated reference voltage V<sub>ref,n</sub>, then the MSB is set to 1. Otherwise, it is set to 0. The process is repeated with the next lower bit, with the new internally generated reference voltage V<sub>ref,n+1</sub> = (½)(V<sub>ref,n</sub>) + V<sub>ref,n</sub> or (½)(V<sub>ref,n</sub>) depending on the previous result, until the results converge, and the conversion is completed.
